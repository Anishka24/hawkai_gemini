...
# Larger chunks reduce the number of embedding calls needed for long rulebooks.
CHUNK_SIZE = 2400
CHUNK_OVERLAP = 150
TOP_K = 6
...
def embedding_values(client: genai.Client, contents: list[str], task_type: str) -> np.ndarray:
    ...
    for attempt in range(5):
        try:
            ...
            # Gemini returns 429 when the free embedding quota is temporarily full.
            if "429" not in str(error) or attempt == 4:
                raise

            wait_match = re.search(r"retry in ([0-9.]+)s", str(error), re.IGNORECASE)
            wait_seconds = float(wait_match.group(1)) + 3 if wait_match else 60
            st.info(
                f"Gemini's free embedding limit was reached. "
                f"Continuing automatically in {wait_seconds:.0f} seconds..."
            )
